...
They FAIL on a runner that only applies ``log1p`` (no ``_preprocess`` symbol /
method != ``normalize_total+log1p`` / per-spot library sizes left intact) and
PASS once #197 wires ``sc.pp.normalize_total`` before ``log1p``.
"""
...
def test_runner_exposes_preprocess():
    """#197 adds a ``_preprocess`` helper; the log1p-only runner has none."""
    mod = _load_runner()
    assert hasattr(mod, "_preprocess"), (
        "run_real_factorgraph must expose _preprocess (normalize_total -> log1p)"
    )
